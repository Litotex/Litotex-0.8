        ...
        <div id="navi_main">
        	<ul>
                ...
                <li{if $item.ID == 4} style="border-right: 0px;"{/if}>
                    <a name="{$item.ID}" title="{$item.title}" rel="{$item.description}" href="index.php?topMenu=1" class="topNavigation {if $item.active}navi_main_active{else}navi_main_show{/if}">
                	{if $item.active}<div class="navi_main_aktiv"></div>{/if}
                	<div>
