...
function smarty_compiler_displayTplModification($params, &$smarty)
{
    ...
    $code = '<?php ob_start(); package::$packages->displayTplModification(\'' . $params['position'] . '\'); $out' . $params['position'] . ' = ob_get_contents(); ob_end_clean(); ?>';
    package::$tpl->addAditionalSmartyHeader($code);
    return 'echo $out' . $params['position'] . ';';
}
