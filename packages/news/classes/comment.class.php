...
class comment {
    ...
    public static function publish($title, $text, $news, $writer) {
        ...
        if(!is_a($writer, 'user'))
                return false;
        $result = package::$db->Execute("INSERT INTO `lttx_newsComments` (`title`, `text`, `date`, `news`, `writer`, `IP`) VALUES (?, ?, " . ", ?, ?, ?", array($title, $text, $news->getID(), $writer->getID(), session::getIPAdress()));
    ...
    public static function getAll($news = false, $page = 1, $offset = false) {
        ...
        if($news)
            $result = package::$db->SelectLimit("SELECT `ID`, `title`, `text`, `date`, `news`, `writer`, `IP` FROM `lttx_newsComments` WHERE `news` = ? ORDER BY `date` DESC", $offset, $start, array($news->getID()));
        else
            $result = package::$db->SelectLimit("SELECT `ID`, `title`, `text`, `date`, `news`, `writer`, `IP` FROM `lttx_newsComments` ORDER BY `date` DESC", $offset, $start);
        if(!$result)
            return false;
        while(!$result->EOF){
    ...
    public static function getNumber($news = false){
        if($news && !is_a($news, 'news'))
                return false;
        if($news)
            $result = package::$db->Execute("SELECT COUNT(`ID`) FROM `lttx_newsComments` WHERE `news` = ?", array($news->getID()));
        else
            $result = package::$db->Execute("SELECT COUNT(`ID`) FROM `lttx_newsComments`");
        if(!$result)
            return false;
    ...
    private function _get($ID) {
        if($this->_getCommentCached($ID))
                return true;
        $ID = (int)$ID;
        $result = package::$db->Execute("SELECT `ID`, `title`, `text`, `date`, `news`, `writer`, `IP` FROM `lttx_newsComments` WHERE `ID` = ?", array($ID));
